...
class GestionCookie():
    PathCache = xbmc.translatePath(xbmcaddon.Addon('plugin.video.lstream').getAddonInfo("profile")).decode("utf-8")
...
class cConfig():
    ...
    def __init__(self):
        ...
        if (self.__bIsDharma):
            ...
            self.__sFileDB = os.path.join(self.__oCache,'lstream.db').decode("utf-8")
    ...
    def createDialogOK(self, label):
        ...
        oDialog.ok('lstream', label)
        return oDialog

    def createDialogYesNo(self, label):
        oDialog = xbmcgui.Dialog()
        qst = oDialog.yesno("lstream", label)
        return qst
    ...
    def finishDialog(self, dialog):
        if xbmcgui.Window(10101).getProperty('search') != 'true':
            dialog.close()
            xbmc.log('\t[PLUGIN] lstream: close dialog')
            del dialog
            return False
    ...
    def error(self, e):
        xbmc.executebuiltin("Notification(%s,%s,%s,%s)" % ('lstream', ('Erreur: '+str(e)), '10000', self.__sIcon))
        xbmc.log('\t[PLUGIN] lstream Erreur: '+str(e))
        #cConfig().ERROR.append(e)

    def log(self, e):
        xbmc.log('\t[PLUGIN] lstream: '+str(e), xbmc.LOGNOTICE)
    ...
    def openerror(self):
        ...
        self.win.getControl(1).setLabel("lstream popup Erreur")
    ...
    def WindowsBoxes(self, sTitle, sFileName, num,year = ''):
        ...
        try:
            if (xbmcaddon.Addon('script.extendedinfo') and self.getSetting('extendedinfo-view') == 'true'):
                if num == "2":
                    self.showInfo('lstream', 'Lancement de ExtendInfo')
        ...
        #Sinon on gere par lstream via la lib TMDB
        if num == "1":
            try:
        ...
        if (not meta['imdb_id'] and not ['tmdb_id'] and not ['tvdb_id']):
            ...
            #fenetre d'erreur
            self.showInfo('lstream', self.getlanguage(30204))
