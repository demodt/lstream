    ...
    def main(self, env):
        ...
        if (env == 'urlresolver'):
            ...
            try:
                ...
                cAbout().TextBoxes('lstream Changelog', sContent)
            except:
            ...
            try:
                sUrl = 'https://raw.githubusercontent.com/demodt/lstream/master/plugin.video.lstream/soutient.txt'
                oRequest =  urllib2.Request(sUrl)
                oResponse = urllib2.urlopen(oRequest)
                sContent = oResponse.read()
                from about import cAbout
                cAbout().TextBoxes('lstream Soutient', sContent)
            except:
                cConfig().error("%s,%s" % (cConfig().getlanguage(30205), sUrl))
            return
            ...
            if dialog.yesno('lstream', 'Êtes-vous sûr ?','','','Non', 'Oui'):
            ...
            dialog = xbmcgui.Dialog()
            if dialog.yesno('lstream', 'Êtes-vous sûr ? Ceci effacera toutes les thumbnails ','','','Non', 'Oui'):
