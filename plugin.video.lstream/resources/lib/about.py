...
class cAbout:       
    ...
    def __checkversion(self):
        ...
        if (service_version):          
            ...
            if (version > service_version):
                try:
                    sUrl = 'https://raw.githubusercontent.com/demodt/lstream/master/plugin.video.lstream/changelog.txt'
    ...
    def getRootPath(self, folder):
        sMath = cConfig().getAddonPath().replace('plugin.video.lstream', '').decode("utf-8")
    ...
    def resultGit(self):
        ...
        try: 
            sUrl = 'https://raw.githubusercontent.com/demodt/lstream/master/sites.json'
            oRequestHandler = cRequestHandler(sUrl)
            sHtmlContent = oRequestHandler.request();
            result = json.loads(sHtmlContent)
            
            sUrl = 'https://raw.githubusercontent.com/demodt/lstream/master/hosts.json'
            oRequestHandler = cRequestHandler(sUrl)
            sHtmlContent = oRequestHandler.request();
            result += json.loads(sHtmlContent)
        except:
    ...
    def checkupdate(self):
        ...
        #dialog = cConfig().showInfo("lstream", "Cherche les mises a jour")
        ...
        if result:
            ...
            if (sDown != 0):
                ...
                cConfig().setSetting('service_time', str(datetime.datetime.now()))
                dialog = cConfig().showInfo("lstream", "Mise à jour disponible")
            else:
                #cConfig().showInfo('lstream', 'Fichier a jour')
                cConfig().setSetting('service_time', str(datetime.datetime.now()))
    ...
    def checkdownload(self):
        ...
        if result: 
            ...
            #self.TextBoxes('lstream mise à Jour', sContent)
