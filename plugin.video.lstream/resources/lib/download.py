...
#     Memorise = StorageServer.StorageServer("lstreamDownloader")
# except:
...
class cDownloadProgressBar(threading.Thread):
    ...
    def _StartDownload(self):
        ...
        #self.Memorise.set("lstreamDownloaderWorking", "1")
        ...
        cConfig().showInfo('lstream', 'Téléchargement Démarré')
        ...
        while not (self.processIsCanceled or diag.isFinished()):
            ...
            #if self.Memorise.get("lstreamDownloaderWorking") == "0":
        ...
        #????????????????
        #Memorise.unlock("lstreamDownloaderLock")
    ...
    def run(self):
        ...
        # if not Memorise.lock("lstreamDownloaderLock"):
...
class cDownload:
    ...
    def isDownloading_old(self):
        ...
        if not Memorise.get('lstreamDownloaderLock'):
            return False
        return True
    ...
    def download(self, sDBUrl, sTitle,sDownloadPath,FastMode = False):
        ...
        #aLink = (True,'https://github.com/LordVenom/venom-xbmc-addons-beta/blob/master/plugin.video.lstream/Thumbs.db?raw=true')
    ...
    def CleanDownloadList(self):
        ...
        try:
            cDb().clean_download()
            cConfig().showInfo('lstream', 'Liste mise a jour')
            #cConfig().update()
        except:
            pass
    ...
    def DelFile(self):
        ...
        if (oDialog == 1):
            ...
            try:
                ...
                xbmcvfs.delete(path)
                cConfig().showInfo('lstream', 'Fichier supprimé')
                cConfig().update()
            except:
                cConfig().showInfo('lstream', 'Erreur, fichier non supprimable')
