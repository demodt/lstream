...
class cTrakt:
    ...
    def getToken(self):
       ...
       # xbmc.log(str(result))
        ...
        if (total > 0):
            ...
            dialog = cConfig().createDialog('lstream')
    ...
    def showHosters(self):
        ...
        ret = dialog3.select('Selectionner un Moteur de Recherche',['lstream (Fiable mais plus complexe)','Alluc (Simple mais resultats non garantis)'])

        if ret == 0:
            self.lstreamSearch(sMovieTitle)
        elif ret == 1:
    ...
    def lstreamSearch(self, sMovieTitle):
        oGui = cGui()
        oInputParameterHandler = cInputParameterHandler()
        sUrl = oInputParameterHandler.getValue('siteUrl')
