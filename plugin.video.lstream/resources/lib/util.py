...
def VSerror(e):
    xbmcgui.Dialog().notification('lstream','Erreur: '+str(e),xbmcgui.NOTIFICATION_ERROR,2000)
    VSlog('Erreur: ' + str(e))
...
def VStranslatePathAddon(location):
    ...
    return xbmc.translatePath(xbmcaddon.Addon('plugin.video.lstream').getAddonInfo(location)).decode("utf-8")
...
def VSlang(lang):
    #util.VSlang(30003)
    #Bug avec accent return xbmc.translatePath(xbmcaddon.Addon('plugin.video.lstream').getLocalizedString(lang)).decode("utf-8")
    return xbmc.translatePath(xbmcaddon.Addon('plugin.video.lstream').getLocalizedString(lang))
